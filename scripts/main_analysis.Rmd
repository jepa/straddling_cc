...
rfmo_prop_data_esm <- bind_rows(
  ...
  # Re- estimate change with average of all ESMs
rfmo_prop_data <- rfmo_prop_data_esm %>%   group_by(taxon_key,taxon_name,common_name,rcp,period,rfmo_t,rfmo_n) %>% 
  summarise_at(
...
sankey_rfmo <- rfmo_prop_data %>% 
  group_by(period,rcp,rfmo_name = rfmo_t,change,name = rfmo_n) %>% 
  tally()
