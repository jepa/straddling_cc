...
length(unique(rfmo_prop_data_esm$taxon_key))# 45
...
x <- rfmo_prop_data_esm %>% 
  group_by(period,rcp,name = rfmo_t,change,rfmo_name = rfmo_n) %>% 
  tally()
...
lapply(rcp_vals, function(rcp) {
  lapply(period_vals, function(period) {
    lapply(change_vals, function(change) {
      circular_plot(data = x, period_val = period, change_val = change, level = level_val, rcp = rcp)
    })
  })
})
...
rfmo_net_change <- rfmo_prop_data_esm %>% 
  group_by(period,rcp,name = rfmo_t,change,rfmo_name = rfmo_n) %>% 
  tally() %>% 
      pivot_wider(
        names_from = change,
    ...
    name_net <- paste0("./results/figures/rfmo_circular_net_","ear","_","rcp26",".png")
    png(name_net,
