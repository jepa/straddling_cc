...
stradd_data <- bind_rows(
  lapply(stradling_spp[1:2],GetResults)
)
...
taxon_proportion <- function(taxon_key,high_migratory){
  ...
  for(m in 1:6){
    for(i in 1:length(fao_areas)){
      ...
      #################################
      # List esm folders
      load(paste0(files_path[m],"/",to_read))
      ...
      # to_read = paste0(taxon_key,"Abd.RData")
      # load(paste0("/Volumes/Enterprise/Data/dbem/dbem_cmip6/C6GFDL26F1_t2/",to_read))
  ...
  return(paste("Data for",taxon_key,esm,rcp, "saved"))
