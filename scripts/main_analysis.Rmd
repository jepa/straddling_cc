...
### Read straddling stocks results
...
Run the function to load results
...
Estimates proportion of abundance in three time periods per taxon

```{r estimate_proportion, eval = F}


taxon_proportion <- function(taxon_key,high_migratory){
  print(taxon_key)
      ...
      for(i in 1:length(fao_areas)){
        ...
        spp_data_area <- spp_data %>% 
          ...
          left_join(meow_dbem_grid,
                    by = "index") %>% 
          mutate(name = ifelse(is.na(name),fao_area,"eezs"),
                 realm = ifelse(is.na(realm),"high seas",realm)
          ) %>% 
          # When straddling use realm when highly migratory just fao region
          {if(high_migratory == F) group_by(.,year,realm) else group_by(., year,name)} %>%
        ...
        hs_historical_means <- spp_proportion %>% 
          filter(year <= 2014) %>%
          {if(high_migratory == F) group_by(.,taxon_key,realm) else group_by(., taxon_key,name)} %>%
        ...
        prop_chng <- spp_proportion %>% 
          ...
          filter(!is.na(period)) %>% 
          {if(high_migratory == F) group_by(.,taxon_key,realm,period) else group_by(., taxon_key,name,period)} %>%
  ...
  # Save file
  name_file <- my_path("R","Per_change", paste0(taxon_key,"_perchg.csv"))
...
suppressMessages(
taxon_proportion(taxon_key = 600089,
        high_migratory = F
        )
)
...
ggplot() + 
  ...
  geom_sf(data = sf_meow %>% left_join(strad_meow), aes(fill = n)) +
  geom_sf(data = sf_land, aes(), fill = "grey30", color = "grey30") +
  scale_fill_viridis_c(" Straddling species (n)") +
  my_ggtheme_m(leg_pos = "right")

  # Save plot 
ggsave("./results/figures/straddling_species.png",
       last_plot(),
       height = 7,
       width = 14
)
