...
length(unique(highly_migratory_spp$taxon_key))# 18 species that are highly migratory
...
plot <-
  ...
  geom_bar(
    aes(
      x = reorder_within(commercial_group,n,realm_or),
      y = round(n),
...
commerica_group_total <- prop_data %>% 
  left_join(sau_groups) %>% 
  group_by(commercial_group,hs_name,realm_name) %>% 
  summarise(n = length(unique(taxon_key))) %>% 
...
sankey_df <- prop_data %>% 
  ...
  # filter(rcp == "rcp85") %>% 
  mutate(
    realm_name = str_replace(realm_name,"SouthernB","S."),
    realm_name = str_replace(realm_name,"NorthernB","N."),
    realm_name = str_replace(realm_name, "WesternB", "W."),
    realm_name = str_replace(realm_name,"EasternB", "E."),
    name = str_replace(realm_name," ", "\n"),
    realm_name = ifelse(name == "Arctic","Arctic ",realm_name)
  ) %>% 
  mutate(
    realm_name = paste0(realm_name,"\n (n =" ,n,")"),
    name = paste0(name,"\n(n =" ,n,")")
  )

# Change names for sankey flow
gain_data <- sankey_df %>% 
  filter(change == 'gain',
         rcp == "rcp26",
         period == "ear") %>% 
...
sankey_data <- bind_rows(gain_data,loss_data)

# Create nodes (unique realm names + hs_name)
nodes <- unique(c(sankey_data$name,sankey_data$hs_name))
nodes <- data.frame(name = nodes) %>% 
  mutate(
    # group = c(rep("in",12),"hs",rep("out",12))
    group = c(gain_data$name,"High Seas",loss_data$name)
  )

# Create links (realm_name -> hs_name for 'gain')
links <- data.frame(
  source = match(sankey_data$name, nodes$name) - 1, 
  target = match(sankey_data$hs_name, nodes$name) - 1,
  value = sankey_data$n,
  group = c(sankey_data$name[1:12],sankey_data$name[1:12])
)

nodes$group <- rep("my_unique_group", 25)
...
# my_color <- 'd3.scaleOrdinal() .domain(["Loss", "Win", "my_unique_group"]) .range(["#69b3a2","steelblue","grey"])'


my_color <- 'd3.scaleOrdinal() .domain(["Loss", "Win", "my_unique_group"]) .range(["#440154FF","#046C9A","#F4B5BD","#9C964A","#D69C4E","#CDC08C","#FAD77B","#DD8D29","#E2D200","#46ACC8","#E58601","#B40F20","#000000","#440154FF","#046C9A","#F4B5BD","#9C964A","#D69C4E","#CDC08C","#FAD77B","#DD8D29","#E2D200","#46ACC8","#E58601","#B40F20","#000000"])'
