...
StraddIndex <- function(Spp, Model = "All", Neighbours, Coord, Index_Code){
  ...
  # Make sure the number of grid cells shared between the high seas and *all* realms is more than 10%
  
  Trans_Spp %>% 
    group_by(category) %>% 
    tally() %>% 
    spread(category,n) %>% 
    mutate(total = realm + rfmo,
           per_tresh_all = rfmo/total*100) %>% 
    View()
  ...
  for(m in 1:6){
    ...
    if(file.exists(file_to_read)){
      ...
      final_data <- prop_chng %>% 
        ...
        select(taxon_key,esm,rcp,period,rfmo_name,change,realm_name,low_tresh,mean,top_tresh, sd) 
...
stradling_spp <- list.files(my_path("R","Straddling_realm_rfmo/"))
stradling_spp <- str_replace(stradling_spp,"\\_.*","")

# Species to ran
proportion_spp <- list.files(my_path("R","Per_change_realm_rfmo"))
proportion_spp <- str_sub(proportion_spp,1,6)

# Run reminindg species
missing_spp <- stradling_spp[!(stradling_spp %in% proportion_spp)]

# Get straddling species
straddling_df <- prop_data <- bind_rows(
  lapply(stradling_spp,GetResults, type = "strad")
  ) %>% 
  rename(realm_name = realm
         ) %>% 
  filter(taxon_key %in% missing_spp) 

# 134 records are under 10%
tresh10_df <- straddling_df%>% 
  mutate(
    tresh10 = ifelse(round(area_rfmo) >= 90 | round(area_rfmo) <= 10, "10per",NA)
  ) 
...
rest %>% pull(taxon_key) %>% unique()
# 600107 600115 600143 600216 600223 600303 600319 600363 600467 600482 600751 600868 600875 600912 601072 601365 601664 601951 602535 603267 604239 614290 690124 690206

# Some of them are in RCP 45
...
length(unique(prop_data$taxon_key)) #98
round(98/633 *100) #15
...
write_csv(straddling_species, my_path("R", name = "straddling_species.csv"))
...
stradd_bar_plot <- stradd_commercial %>% 
  ...
  scale_fill_manual("Commercial group",values = c(wes_palette("Darjeeling1"),wes_palette("Darjeeling2"))) +
...
#   select(-total_changes) %>% 
  # write_csv(.,my_path("R", name = "gains_losses.csv"))
...
missing_data <- sankey_df %>% 
  select(realm_name) %>% 
  mutate(rowname = str_replace(realm_name," ", "\n")) %>% 
  filter(rowname %in% circle_data$rowname)
...
n_labels = length(c(unique(circle_data$key),unique(circle_data$rowname)))

if(n_labels == 17){
  
  # Total colors 17
  mycolor_scale <- c(
    viridis(8, alpha = 1, begin = 0, end = 1, option = "D"),
    viridis(12, alpha = 1, begin = 0, end = 1, option = "D")[8:11],
    viridis(5, alpha = 1, begin = 0, end = 1, option = "F")
  )
  
}else{
  
  mycolor_scale <- c(viridis(11, alpha = 1, begin = 0, end = 1, option = "D"),
                 viridis(5, alpha = 1, begin = 0, end = 1, option = "F")
  )
...
# png("./results/figures/circular_same.png", width = 1300, height = 1300, res = 150, units = "px")
chordDiagram(
