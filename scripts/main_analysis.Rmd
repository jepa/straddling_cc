...
EEZ_CellID <- my_path("Spa","DBEM", "EEZ_CellID.xlsx", read = TRUE,system = "jepa")
colnames(EEZ_CellID) <- c("EEZID","index")

# Now our gridcell
abny <- my_path("Spa","DBEM","Lon_Lat_DBEM.txt",read = T, header = F, system = "jepa") %>% 
  bind_cols(my_path("Spa","DBEM","WArea.txt",read = T, header = F, system = "jepa") %>% slice(-259201)) %>% 
  filter(!index %in% EEZ_CellID$index) %>% 
  mutate(cat = ifelse(v1>0, "abnj","land")) %>% 
  select(-v1)

# SAVE FOR COMPLETE DBEM
complete_lon_lat_dbem <- my_path("Spa","DBEM","Lon_Lat_DBEM.txt",read = T, header = F, system = "jepa") %>% 
  left_join(abny) %>% 
