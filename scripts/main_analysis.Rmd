...
intense_data <- prop_data %>% 
  ...
  mutate(
    ...
    mean_eez = 100-mean,
    mean_intensity_hs = my_chng(hs_hist_prop,mean,100),
    mean_intensity_eez = my_chng(eez_hist_prop,mean_eez,100)
  ) %>% 
  # mutate_if(is.numeric,round) %>% 
  # View()
  left_join(sau_groups) %>% 
  # View()
  group_by(commercial_group,period,rcp,realm_name,change) %>%
  summarise(
    mean_intensity = mean(mean_intensity_eez,na.rm = T),
    sd_intensity = sd(mean_intensity_eez,na.rm = T)
  ) %>% 
    filter(mean_intensity != 0)

# intense_data %>% 
#   filter(rcp == "rcp26") %>%
#   # filter(realm_name %in% c("Central Indo Pacific","Tropical Eastern Pacific")) %>% 
#   # View()
#   pull(commercial_group) %>% 
#   unique()
#   
unique(prop_data$realm_name)
...
intense_data %>% 
  ...
  geom_bar(
    aes(
      # x = commercial_group,
      x = tidytext::reorder_within(commercial_group,mean_intensity,realm_name),
      y = mean_intensity,
      color = realm_name,
      fill = realm_name
    ),
  ...
  geom_errorbar(
    aes(
      x = reorder_within(commercial_group, mean_intensity, realm_name), 
  ...
  coord_flip() +
  scale_color_manual("Region", values = mycolor_scale) +
  scale_fill_manual("Region", values = mycolor_scale) +
...
intense_data <- prop_data %>% 
  ...
  # View()
  left_join(sau_groups) %>%
  # select(taxon_key,rcp,period,hs_name,realm_name,hs_hist_prop,eez_hist_prop,hs_mean = mean,eez_mean,commercial_group) %>% 
  group_by(rcp,period,hs_name,realm_name,commercial_group) %>% 
  summarise_if(is.numeric,mean) %>% 
  rename(
    hist_hs_prop = hs_hist_prop,
    hist_eez_prop = eez_hist_prop,
    med_hs_mean = mean,
    med_eez_mean =eez_mean
  ) %>% 
  gather("time","vale",hist_hs_prop,hist_eez_prop,med_hs_mean,med_eez_mean)
