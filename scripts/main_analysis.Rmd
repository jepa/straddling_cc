...
ob_index <- rfmo_index%>% 
  mutate(
    basin = ifelse(rfmo_name == "ICCAT","atlantic",
                   ifelse(rfmo_name == "WCPFC","pacific",
                          ifelse(rfmo_name == "IATTC","pacific",
                                 ifelse(rfmo_name == "IOTC","indian",
                                        ifelse(rfmo_name == "CCSBT","southern","NA")))
  )))
...
# This data now substitutes rfmo_meow
basin_data <- rfmo_meow %>%
  mutate_at(vars(region, adjacent_region),
            ~ ifelse(. == "ICCAT","atlantic",
                     ifelse(. == "WCPFC","pacific",
                            ifelse(. == "IATTC","pacific",
                                   ifelse(. == "IOTC","indian",
                                          ifelse(. == "CCSBT","southern",.)))
                     )
            )
  )

# Remove them to make sure analysis is correct
rm(rfmo_meow,rfmo_index)
...
length(unique(partial_straddling_df$taxon_key))# 157
...
```{r threshold_sensitivity, eval = F}
 ...
 bind_rows(
  ...
  mutate(
    tresh_30 = ifelse(area_rfmo > 30, "30%",NA),
    tresh_20 = ifelse(area_rfmo > 20, "20%",NA),
    tresh_0 = ifelse(area_rfmo > 0, "0%",NA),
    tresh_10 = ifelse(area_rfmo > 10, "10%",NA),
    combo = paste(realm_name,rfmo_name, sep = "-")
  ) %>%
  # View()
  group_by(combo,tresh_30,tresh_20,tresh_10,tresh_0) %>%
    tally() %>%
  # View()
...
for(i in 1:length(unique(stradd_commercial_df$realm_name))){
  ...
  plot <- stradd_commercial_df %>% 
    ...
    facet_grid(rfmo_name~realm_name, switch = "x",
               scales = "free") +
    labs(x = "", y = "") +
  ...
  plot_name <- paste0("./results/figures/fig_1_singles/",realm_or_name,".png")
...
commerica_group_total <- prop_data %>% 
  left_join(sau_groups) %>% 
  group_by(commercial_group,rfmo_name,realm_name) %>% 
  summarise(n = length(unique(taxon_key))) %>% 
...
prop_data %>% 
  left_join(sau_groups) %>% 
  # mutate(straddling_status = ifelse(reporting == "","Not-known","Known")) %>% 
  ...
  mutate(
    per = n/tot*100,
    change = ifelse(change == "gain","RFMO",
                         ifelse(change == "lost","EEZ","Same")),
    period = ifelse(period == "ear","Early","Middle"),
    rcp = ifelse(rcp == "rcp26","SSP1-2.6","SSP5-8.5")#,
    # x_lab = paste0(unique(commercial_group)," (",unique(tot),")")
         ) %>% 
  # filter(change != "Same") %>%
  # View()
  ggplot() +
...
lapply(rcp_vals, function(rcp) {
  lapply(period_vals, function(period) {
    lapply(change_vals, function(change) {
      circular_plot(data = sankey_df, period_val = period, change_val = change, rcp = rcp)
    })
  })
})
...
regional_map <- ggplot() + 
  geom_sf(data = sf_rfmo %>% left_join(stradd_rfmo), aes(),fill = "transparent") +
