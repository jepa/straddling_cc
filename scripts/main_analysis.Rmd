...
my_sf("MEOW") %>% 
  ggplot() +
  geom_sf(aes(fill = REALM, color = REALM))
      ...
      for(i in 1:length(fao_areas)){
        ...
        spp_data_area <- spp_data %>% 
          ...
          mutate(name = ifelse(is.na(name),fao_area,"eezs"),
                 realm = ifelse(is.na(ecoregion),"high seas",realm)
          ) %>% 
          # When straddling use realm when highly migratory just fao region
          {if(!is.na(category)) group_by(.,year,fao_area_name) else group_by(., year,realm)} %>%
        ...
        hs_historical_means <- spp_proportion %>% 
          filter(year <= 2014) %>%
          {if(!is.na(category)) group_by(.,taxon_key,ecoregion) else group_by(., taxon_key,realm,name)} %>%
        ...
        prop_chng <- spp_proportion %>% 
          ...
          filter(!is.na(period)) %>% 
          {if(!is.na(category)) group_by(.,taxon_key,ecoregion,period) else group_by(., taxon_key,realm,name,period)} %>%
