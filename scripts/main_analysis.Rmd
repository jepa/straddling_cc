...
colnames(ob_meow)[2] <- "neighbouring_region"
...
partial_straddling_df_ob <- bind_rows(
  lapply(straddling_spp,GetResults, type = "ob")
) %>% 
  mutate(approach = "ocean_basin") %>% 
  rename(realm_name = realm)

length(unique(partial_straddling_df$taxon_key))# 27

# Get straddling stocks
prop_data_esm_ob <- bind_rows(
  lapply(unique(partial_straddling_df_ob$taxon_key),GetResults, type = "prop_ob")
) %>% 
  # View()
  # Include area index info
  # left_join(partial_straddling_df_ob,
  #           by = c("taxon_key","hs_name","realm_name")
  #       ) %>% 
  filter(area_hs > 10,
         area_hs < 90) %>% 
...
```{r}
bind_rows(prop_data_ob,
          prop_data %>% filter(taxon_key %in% prop_data_ob$taxon_key)) %>% 
  group_by()
