...
length(unique(straddling_hm$taxon_key))# 19 species that are highly migratory
...
length(unique(prop_data_hm$taxon_key))# 15 #Number that changed 15/19 (79%)
...
length(unique(rfmo_prop_data$taxon_key)) # 13 species that changed
...
# The RFMO shapefile needs to be re-manipulated
...
#   # st_wrap_dateline(options = c("WRAPDATELINE=YES",  "DATELINEOFFSET=180"), quiet = T) %>%
#   group_by(rfmo_nm_s) %>% 
#   summarise(geometry = st_union(geometry)) 
#   st_transform(crs = "+proj=eck4") %>%
#   st_shift_longitude()
...
map_high_seas_stradd <-
  ...
  geom_sf(data = strad_meow_map,
          aes(fill = n)) +
  # geom_sf(data = sf_rfmoc, aes(), fill = "transparent", color = "black") +
  geom_sf(data = sf_land, aes(), fill = "antiquewhite", color = "grey50") +
  # Add RFMO labels
  # geom_label_repel(data = sf_rfmoc,
  #                  aes(label = rfmo_name,
  #                      geometry = geometry),
  #                  stat = "sf_coordinates",
  #                  direction = "y",
  ...
  #                  point.padding = 50,
  #                  hjust = "right",
  #                  nudge_x = ifelse(sf_rfmoc$rfmo_name == "ICCAT",-13,
  #                                   ifelse(sf_rfmoc$rfmo_name == "WCPFC",10,-5)),
  #                  nudge_y = ifelse(sf_rfmoc$rfmo_name == "CCSBT",-20,
  #                                   ifelse(sf_rfmoc$rfmo_name == "ICCAT",10,
  #                                          ifelse(sf_rfmoc$rfmo_name == "WCPFC",10,1))),
  #                  alpha = 0.6,
  #                  size = 4
  # ) +
...
# Save plot 
ggsave("./results/figures/fig_1an.png",
       map_high_seas_stradd,
       height = 7,
       width = 12
)
...
ggsave(
  "./results/figures/per_chng/per_chng_earb.png",
       last_plot(),
       width = 7,
       height = 8)
