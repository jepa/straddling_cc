...
# rfmo_sf %>%
#   ggplot() +
#   geom_sf(aes(fill = RFMO_nm_s))


rfmo_index <- read_csv("~/Library/CloudStorage/OneDrive-UBC/Data/Spatial/SAU/RFMO_Index_Code.csv") %>% 
  filter(Territory %in% c("ICCAT","WCPFC","IOTC","IATTC","CCSBT")) %>% 
  clean_names()%>% 
  rename(rfmo_name = territory)


fao_index <- my_path("Spa", "DBEM","fao_to_dbem_grid.csv", read = T)


rfmo_to_fao_index <- rfmo_index %>% 
  left_join(fao_index,
            by ="index") %>% 
  rename(rfmo_name = territory)

rfmo_to_fao <- rfmo_to_fao_index %>% 
  group_by(rfmo_name,fao_area_name) %>% 
  tally() %>% 
  select(-n)
...
StraddIndex <- function(Spp,Model = "All",Neighbours,Coord,Index_Code){
  ...
  Trans_Spp <- SppDist %>%
    ...
    left_join(rfmo_index,
              by = "index") %>% 
    mutate(
      territory = ifelse(is.na(rfmo_name),realm,rfmo_name),
  ...
  if("rfmo" %in% Trans_Spp$category == FALSE){
    ...
    Neighbours_List <- neighbours %>%
      ...
      ungroup() %>%
      select(-n) %>% 
      left_join(rfmo_to_sau) %>% 
      filter(!is.na(territory))
    
    # Set RFMO neighbours... 
    rfmo_neighbours <- Neighbours_List %>% 
      select(territory, rfmo) %>% 
      rename(territory = rfmo,
             neighbour = territory) %>% 
      group_by(territory, neighbour) %>% 
      summarise(n())
    
    
    rfmo_territory <- rfmo_neighbours %>% 
      rename(territory = neighbour,
             neighbour = territory)
    
    
    neighbours_list_rfmo <- rfmo_neighbours %>% 
      bind_rows(Neighbours_List,rfmo_territory) %>% 
      select(-rfmo)
    ...
    Spp_Grid <- Trans_Spp %>% 
      ...
      left_join(neighbours_list_rfmo,
                by = "territory") %>% 
    ...
    Area_Index_D <- full_join(Territory_T,
      ...
      rename(territory = Name,
             ...
             n_spp_rfmo = n_spp_eez.z) %>%
      # Estimate if it is straddling
      mutate(stradling_index = (n_spp_rfmo/spp_total)*100)
    ...
    Save_Path <- my_path("R","Straddling_rfmo", File_Name)
...
# RFMO data
rfmo_index <- my_path("D","/Spatial/","RFMO_Index_Code.csv", read = T)
...
rfmo_to_sau <- read_csv("~/Library/CloudStorage/OneDrive-UBC/Data/Spatial/SAU/RFMO_Index_Code.csv") %>% 
  filter(Territory %in% c("ICCAT","WCPFC","IOTC","IATTC","CCSBT")) %>% 
  clean_names() %>% 
  left_join(sau_index_code) %>% 
  group_by(rfmo = territory,
           territory = name) %>% 
  tally() %>% 
  select(-n)
...
neighbours <- my_path("G","FishForVisa/Data/Spatial/", name = "EEZ_Neighbour_List.xlsx", read = T) %>%
  clean_names() %>% 
  rename(eez_id = eezid) %>% 
  left_join(rfmo_index,
            by = "index")
...
run_species <- list.files(my_path("R","Straddling_rfmo"))
...
# lapply(run_me, StraddIndex, Model = "All", Coord = coords_dbem, Index_Code = sau_index_code)

# library(doParallel)
clb <- makeCluster(10) # total 12
# registerDoSNOW(cl)

repetitions <- length(run_me)
pb2 <- txtProgressBar(max = repetitions, style = 3)
...
GetResults=function(Spp,type = "strad"){
  ...
  if(type == "strad"){
    ...
    Distpath <- paste(my_path("R","Straddling_rfmo/"),Spp,"_straddling.csv",sep="")
  }else{
...
taxon_proportion <- function(taxon_key){
  ...
  for(m in 1:6){
    ...
    if(file.exists(file_to_read)){
      ...
      for(i in 1:length(rfmo_areas)){
        # i = 1
        
        rfmo_area <- rfmo_areas[i]
        
        # Get area of analysis 
        taxon_rfmo_dbem_area <- stradd_data %>% 
          filter(rfmo_name %in% rfmo_area) %>% 
          left_join(rfmo_index,
                    by = "rfmo_name") %>% 
          select(index,rfmo_name)
        
        spp_data_area <-
          spp_data %>% 
          filter(index %in% taxon_rfmo_dbem_area$index) %>% 
          ...
          left_join(rfmo_index) %>% 
          mutate(name = ifelse(is.na(eco_code),rfmo_name,realm)) %>% 
        ...
        prop_chng <- spp_proportion %>% 
          ...
          # remove stuff
          mutate(keep = ifelse(name == rfmo_area, "n","y")) %>% 
          filter(keep == "y") %>% 
          select(-keep)
...
spp_complete <- list.files(my_path("R","Straddling_rfmo/"))
...
rfmo_meow <- meow_dbem_grid %>% 
  left_join(rfmo_index) %>% 
  group_by(realm,rfmo_name) %>%
  filter(!is.na(rfmo_name)) %>% 
  tally() %>% 
  select(-n)

strad_rfmo <- shared_data %>%
...
map_rfmo_stradd <- ggplot() + 
  geom_sf(data = sf_rfmo %>%  left_join(strad_rfmo), aes(fill = n,color = n)) +
  ...
  scale_color_viridis_c("Straddling spp.\n(n)", na.value = "white") +
  theme_bw() +
  theme(legend.title.align = 0.5) 

# Save plot 
ggsave("./results/figures/straddling_id/straddling_species_rfmo.png",
       map_rfmo_stradd,
       height = 6,
       width = 12
)
...
shared_data %>% 
  left_join(meow_dbem_eez) %>% 
  group_by(common_name,rfmo_name) %>% 
...
plot_data <- prop_data %>% 
  group_by(name,rfmo_area,esm,rcp,change) %>% 
  tally() %>% 
  group_by(name,rfmo_area,rcp,change) %>% 
