...
circle_data <-
  ...
  filter(
    ...
    group_by(period,rcp) %>% 
    summarise(n_eez = length(unique(realm_name)),
              n_rfmo = length(unique(rfmo_name))) %>% 
  View()

# Color scale for gains

length(unique(circle_data$key)) #5
length(unique(circle_data$rowname)) #11

# Total colors 17
mycolor_g <- c(viridis(11, alpha = 1, begin = 0, end = 1, option = "D"),
             viridis(5, alpha = 1, begin = 0, end = 1, option = "F"),
             viridis(1, alpha = 1, begin = 0, end = 1, option = "F")
             )
