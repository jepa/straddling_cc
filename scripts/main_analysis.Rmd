...
x <- prop_data %>% 
  ...
  # Estimte intensity
  mutate(
    min_intensity = my_chng(low_tresh,mean,100),
    max_intensity = my_chng(top_tresh,mean,100)
  ) %>% 
  # View()
  left_join(sau_groups) %>%
  # View()
  ...
  summarise(
    min_intensity = mean(min_intensity,na.rm = T),
    max_intensity = mean(max_intensity,na.rm = T)
  ) %>%
    # View()
  gather("cat","val",min_intensity:max_intensity) %>% 
  # View()
  ggplot() +
geom_bar(
    aes(
