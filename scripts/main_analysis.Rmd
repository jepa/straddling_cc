...
dbem_sf <-st_as_sf(complete_lon_lat_dbem,
                           coords = c("lon", "lat")
) %>% 
  st_set_crs(4326) #%>%
  # st_transform(crs = "+proj=eck4")
...
fao_grid <- dbem_fao_sf %>% 
  filter(!is.na(F_CODE)) %>% 
  as.data.frame() %>% 
  select(index,cat,fao_area_code=F_CODE,area_code=F_CODE,ocean=OCEAN,name = NAME_EN) %>% 
  filter(cat == "abnj") %>% 
...
EstPropChange <- function(taxon_key,
  ...
  # Loads species data for runs
  to_read = paste0(taxon_key,"Abd.RData")
  load(paste0("/Volumes/Enterprise/Data/MigrantFish/Data/",to_read))
  ...
  spatial_subset <- stradd_data %>% 
    rename(name = territory) %>% 
    left_join(sau_index_code)
  
  areas_present <- stradd_data %>% 
    gather(territory,fao_area_name, 2,4) %>% 
    pull(fao_area_name) %>% 
      unique()
  
  spp_stradd_range <- sau_fao_grid %>% 
    filter(name %in% areas_present)
  ...
  complete_data <- spp_data %>%
    mutate(index = as.character(index)) %>% 
    # gather("year","value",`1951`:`2100`) %>% 
      left_join(spp_stradd_range,
                by ="index") %>% 
    filter(!is.na(value),
           !is.na(name))
  ...
  # Get neighbouring id
  Neighbours_List <- neighbours %>% 
    semi_join(stradd_data,
              by = c("territory","neighbour","fao_area_name")) %>% 
    group_by(territory,neighbour,fao_area_name) %>% 
    tally()
  ...
  total_by_unit <- complete_data %>% 
    group_by(territory = name,
             area_code,
             cat,
             year) %>% 
    summarise(total_value = sum(value)) %>% 
    left_join(Neighbours_List) %>% 
    mutate(fao_area_name = ifelse(is.na(fao_area_name),territory,fao_area_name))
  
  
  
  # Test
  total_test <- total_by_unit %>% 
    # filter(territory %in% c("Mexico (Pacific)","USA (West Coast)","Pacific, Northeast")) %>% 
    filter(fao_area_name %in% "Pacific, Western Central") %>%
    group_by(year) %>% 
    summarise(total = sum(total_value))
  
  test_t <- total_by_unit %>% 
    # filter(territory %in% c("Mexico (Pacific)","USA (West Coast)","Pacific, Northeast")) %>%
    filter(fao_area_name == "Pacific, Western Central") %>%
    group_by(territory,year) %>% 
    summarise(total_ter = sum(total_value)) %>% 
    left_join(total_test) %>% 
    mutate(per = total_ter/total) %>% 
    group_by(territory) %>% 
    summarise(mean = mean(per),
              sd = sd(per))
  
  historic <- test_t %>% 
    mutate(h_high = mean+sd,
           h_low = mean-sd) %>% 
    select(territory,h_high,h_low)
  ...
  # Example Plot
   ...
   total_by_unit %>% 
    ...
    left_join(total_test) %>% 
    mutate(per = total_ter/total,
           period = ifelse(year < 2014,"hist",
                           ifelse(year >2020 & year <2040,"ear",
