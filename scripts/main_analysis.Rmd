...
fao_grid <- as.data.frame(joint_sf) %>% 
 select(index,NAME_EN,F_CODE) %>% 
  left_join(coords)

# SAVE ME 

# complete_lon_lat_dbem_fao <- complete_lon_lat_dbem %>% 
  left_join(fao_grid)

# Save for fouture
# write_csv(complete_lon_lat_dbem_fao, my_path("Spa", "DBEM","complete_lon_lat_dbem.csv"))
...
## Neighbours and the FAO regions
...
# Load SAU shapefile (could take a while)
sau_sf <- my_sf("SAU")
...
sau_fao_sf <- st_join(sau_sf_b,fao_sf_b)



# coord_sau_fao <- Coord %>%
#   left_join(complete_sau,
#             by = "index") %>% 
#   mutate(name = ifelse(cat == "eez",eez,name_en),
#          spat_code = ifelse(cat == "eez",sau_code,f_code)) %>% 
#   select(-name_en,-f_code,-sau_code,-eez)

# sau_fao_sf <- st_as_sf(coord_sau_fao,
#                  coords = c("lon", "lat")
# ) %>% 
  # st_set_crs(4326)
...
Index_Code <- coord_sau_fao



sau_combo <- read_excel("/Volumes/DATA/JULIANO_NEYMAR/Spatial/SAU/EEZ-FAO-Country-Combo.xlsx") 

fao_to_sau <- sau_combo %>% 
...
sau_fao_df <- as.data.frame(sau_fao_sf) %>% 
  select(-geometry) %>% 
  left_join()
  
  
eez_fao <- sau_combo %>% 
  select(eez_id,eez_name,fao_id,fao_area_name)
...
StraddIndex <- function(Spp,Model = "All",Neighbours,Coord,Index_Code,Save="Y",Jurisdiction_Type){
